            ...
            <div class="w-full flex flex-col" style="height: 420px;">
                <div class="flex flex-col h-full">
                    ...
                    <div class="flex items-center justify-between px-4 py-3 border-b shrink-0">
                        ...
                        <button type="button" wire:click="$set('showCategoryPicker', false)" class="p-1.5 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors">
                    ...
                    <div class="flex-1 overflow-y-auto px-4 py-4">
                                        ...
                                        <div class="col-span-3">
                                            ...
                                            <div class="grid grid-cols-3 gap-2">
                                                @foreach($category->children as $child)
                                                    <button type="button" wire:click="selectCategory({{ $child->id }})" class="p-3 bg-stone-50 dark:bg-stone-800 hover:bg-stone-100 dark:hover:bg-stone-700 rounded-xl transition-all text-center group active:scale-95">
                                                        <div class="flex justify-center mb-1.5">
                                                            <x-dynamic-component :component="'heroicon-o-' . ($child->icon ?? 'folder')" class="w-7 h-7 text-stone-600 dark:text-stone-300 group-hover:scale-110 transition-transform" />
                                                        </div>
                                                        <div class="text-xs font-medium leading-tight text-stone-800 dark:text-stone-200">{{ $child->name }}</div>
                                                    </button>
                                                @endforeach
                                            </div>
                                        </div>
                                    @else
                                        <button type="button" wire:click="selectCategory({{ $category->id }})" class="p-3 bg-stone-50 dark:bg-stone-800 hover:bg-stone-100 dark:hover:bg-stone-700 rounded-xl transition-all text-center group active:scale-95">
                                            <div class="flex justify-center mb-1.5">
                                                <x-dynamic-component :component="'heroicon-o-' . ($category->icon ?? 'folder')" class="w-7 h-7 text-stone-600 dark:text-stone-300 group-hover:scale-110 transition-transform" />
                                            </div>
                                            <div class="text-xs font-medium leading-tight text-stone-800 dark:text-stone-200">{{ $category->name }}</div>
                                        </button>
                                    @endif
                                @endforeach
                            </div>
                        @else
                            <div class="flex flex-col items-center justify-center h-full py-12">
                                ...
                                <p class="font-medium text-sm text-stone-500 dark:text-stone-400">還沒有建立對應分類</p>
                    ...
                    <div class="px-4 py-3 border-t shrink-0">
                        <button type="button" wire:click="$set('showCategoryPicker', false)" class="w-full py-2.5 rounded-xl font-medium text-sm bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-300"> 取消 </button>
                    </div>
                </div>
            </div>
        @else
            ...
            <div class="grid grid-cols-4 gap-1.5 mb-5">
                ...
                @foreach(['expense' => '支出', 'income' => '收入', 'transfer' => '轉帳'] as $value => $label)
                    <button type="button" wire:click="$set('type', '{{ $value }}')" class="py-2.5 text-sm font-bold rounded-xl transition-all duration-200 {{ $currentType === $value ? $typeClasses[$value] . ' shadow-md' : 'bg-stone-50 dark:bg-stone-800 text-stone-600 dark:text-stone-300' }}">
                        {{ $label }}
                    </button>
                @endforeach
            </div>
            ...
            <x-form wire:submit="saveTransaction">
                ...
                <div class="flex items-center justify-between mb-4 px-2 py-1 rounded-xl">
                    <button type="button" wire:click="changeDate(-1)" class="p-2 hover:bg-stone-200 dark:hover:bg-stone-800 rounded-full transition-colors">
                ...
                @if($type === 'transfer')
                    ...
                    <div class="grid grid-cols-5 gap-3 mb-4 items-end">
                        ...
                        <div class="col-span-1 flex justify-center pb-1">
                            <button type="button" wire:click="swapAccounts" class="p-2 rounded-full hover:bg-stone-100 dark:hover:bg-stone-800" title="切換帳戶">
                        ...
                        <div class="col-span-2">
                            ...
                            <select wire:model.live="toAccountId" class="select select-bordered w-full h-11 text-sm rounded-xl dark:bg-stone-900 dark:text-stone-100">
                                <option value="" class="bg-white dark:bg-stone-900"> 選擇目標帳戶 </option>
                                @foreach($this->accounts as $account)
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <div class="relative w-full flex items-center px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-900/50">
                                ...
                                <input 
                                    ...
                                    placeholder="0" 
                                    autocomplete="off" 
                                    class="w-full pl-1 pr-6 font-bold bg-transparent focus:outline-none focus:ring-2 focus:ring-sky-500/20 rounded text-right caret-stone-900 dark:caret-stone-100 text-lg text-tx-expense" 
                                />
                            </div>
                        </div>
                        {{-- 轉入金額 (示例) --}}
                        <div>
                            ...
                            <div class="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-900/50">
                ...
                @else
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-4">
                        ...
                        <div class="col-span-1">
                            <div class="aspect-square rounded-2xl border border-stone-300 dark:border-stone-700 transition-all cursor-pointer flex flex-col items-center justify-center p-1 relative overflow-hidden bg-stone-50 dark:bg-stone-800 hover:border-stone-400" wire:click="openCategoryPicker(false)">
                                @if($categoryId && $selectedCategory && $selectedCategory->type === $type)
                                    ...
                                    @if($selectedCategory->parent)
                                        <div class="text-[10px] text-stone-500 dark:text-stone-400 mt-0.5"> {{ $selectedCategory->parent->name }} </div>
                                    @endif
                                @else
                                    <div class="text-3xl mb-1 flex justify-center">
...
/>
                ...
                <div class="grid grid-cols-5 gap-3 mb-5">
                    <div class="col-span-2">
                        <label class="aspect-square rounded-2xl border-2 border-dashed border-stone-300 dark:border-stone-700 hover:border-stone-400 dark:hover:border-stone-500 transition-all cursor-pointer flex flex-col items-center justify-center relative overflow-hidden bg-stone-50/50 dark:bg-stone-900/50">
                            ...
                            @else
                                <svg class="w-8 h-8 mb-1 text-stone-400 dark:text-stone-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                <span class="text-xs font-medium text-stone-500 dark:text-stone-400">照片</span>
                            @endif
                    ...
                    <div class="col-span-3">
                        <div class="relative w-full h-full min-h-[80px] px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-900/50">
                            <textarea wire:model="memo" placeholder="請輸入備註" class="w-full h-full bg-transparent text-stone-900 dark:text-stone-100 placeholder:text-stone-400 dark:placeholder:text-stone-500 focus:outline-none placeholder:font-medium resize-none text-sm" style="min-height: 60px;"></textarea>
                        </div>
                    </div>
                </div>
                ...
                <x-slot:actions>
                    @if(!$transactionId)
                        {{-- 新增模式：存為範本、再記一筆、儲存 --}}
                        <div class="grid grid-cols-3 gap-2 w-full pt-3 border-t border-stone-200 dark:border-stone-700">
            ...
            <x-form wire:submit="saveAsTemplate">
                ...
                <div class="grid grid-cols-6 gap-3 mb-4">
                                ...
                                @if($templateCategoryId && $selectedTemplateCategory && $selectedTemplateCategory->type === $templateType)
                                    <div class="text-3xl mb-1 flex justify-center">
                                        <x-dynamic-component :component="'heroicon-o-' . ($selectedTemplateCategory->icon ?? 'folder')" 
                                                class="w-10 h-10 {{ match($templateType) { 'expense' => 'text-tx-expense', 'income' => 'text-tx-income', default => 'text-stone-700 dark:text-stone-200' } }}" />
                                    </div>
                                    <div class="text-xs font-bold text-center leading-tight {{ match($templateType) { 'expense' => 'text-tx-expense', 'income' => 'text-tx-income', default => 'text-stone-800 dark:text-stone-200' } }}">
                    ...
                    <div class="col-span-4 flex items-center">
                        <div class="relative w-full flex items-center bg-stone-50/50 dark:bg-stone-900/50 px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700">
                            <span class="text-2xl font-bold shrink-0 mr-1 {{ $signColor }}">{{ $sign }}</span>
                            <input type="text"
                                   inputmode="decimal"
                                   wire:model.live.debounce.500ms="amount"
                                   ...
                                   class="w-full pl-1 pr-6 font-bold bg-transparent focus:outline-none focus:ring-2 focus:ring-sky-500/20 rounded
                                          placeholder:text-stone-400 dark:placeholder:text-stone-200 text-right
                                          caret-stone-900 dark:caret-stone-100 {{ $signColor }}"
                ...
                <div class="grid grid-cols-4 gap-3 mb-4">
                    <div class="col-span-2">
                        <select 
                            ...
                            class="select select-bordered w-full h-11 text-sm rounded-xl dark:bg-stone-900 dark:text-stone-100">
                            <option value="" class="bg-white text-stone-400 dark:bg-stone-900 dark:text-stone-500">
                                選擇帳戶
                            </option>
                            @foreach($this->accounts as $account)
                                <option value="{{ $account['id'] }}" class="bg-white text-stone-900 dark:bg-stone-900 dark:text-stone-100">
                                    {{ $account['name'] }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-5 gap-3 mb-4">
                    <div class="col-span-5">
                        <div class="relative w-full bg-stone-50/50 dark:bg-stone-900/50 px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-700">
                            <textarea wire:model="templateMemo"
                                      placeholder="請輸入備註"
                                      rows="3"
                                      class="w-full bg-transparent focus:outline-none
                                             text-stone-600 dark:text-stone-100 
                                             placeholder:text-stone-400 dark:placeholder:text-stone-200 
            ...
            @forelse($templates as $template)
                @php
                    $templateTypeColor = match($template['type'] ?? 'expense') {
                        ...
                        'income'   => 'text-tx-income',
                        'transfer' => 'text-tx-transfer',
                        default    => 'text-stone-800 dark:text-stone-100',
                    };
                @endphp
