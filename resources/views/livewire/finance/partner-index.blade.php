...
<div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
	...
	<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-base-200 pb-4">
		<div>
			...
			<p class="text-xs text-base-content/60 mt-1">點擊卡片任一處即可編輯</p>
		</div>
		<div class="flex items-center gap-2 w-full sm:w-auto">
			<x-input wire:model.live.debounce.400ms="search" icon="o-magnifying-glass" placeholder="搜尋姓名..." class="rounded-xl flex-1 sm:flex-initial" clearable />
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
		@forelse($partners as $partner)
			{{-- 整張卡片可點擊開啟編輯 --}}
			<div 
				class="group bg-base-200/50 rounded-2xl p-4 shadow-sm hover:shadow-md border border-transparent hover:border-base-300 transition-all duration-200 cursor-pointer active:scale-[0.98]"
				wire:click="openEdit({{ $partner->id }})"
			>
				{{-- 卡片內容 --}}
				<div class="flex items-start gap-3">
					{{-- 頭像 --}}
					<div class="w-14 h-14 rounded-full border border-base-200 shadow-sm overflow-hidden flex-shrink-0 group-hover:border-teal-200 transition-colors">
						<img 
					...
					<div class="flex-1 min-w-0 space-y-1">
						<div class="flex items-center justify-between gap-2">
							<span class="font-bold text-base text-base-content truncate">{{ $partner->name }}</span>
							<span class="text-[10px] uppercase font-bold tracking-wider text-base-content/60 shrink-0">{{ $partner->role }}</span>
						</div>
						
						<p class="text-xs text-base-content/60 truncate">{{ $partner->user?->email ?? '未綁定帳號' }}</p>
				...
				{{-- 卡片底部：刪除按鈕（獨立避免觸發編輯） --}}
				<div class="flex justify-end mt-3 pt-2 border-t border-base-200">
					<x-button 
						icon="o-trash" 
						class="btn-red" 
		...
		@empty
			...
			<div class="col-span-full">
				<div class="text-center py-16 bg-base-200/30 rounded-2xl border-2 border-dashed border-base-300">
					<x-icon name="o-user-group" class="w-12 h-12 text-base-content/40 mx-auto mb-3" />
					<p class="text-base-content/60 font-medium">還沒有家庭成員</p>
					<p class="text-xs text-base-content/40 mt-1">點擊「建立成員」開始新增</p>
				</div>
			</div>
		@endforelse
	</div>
	...
	<x-drawer 
		...
		separator 
		with-close-button 
		class="w-full sm:w-11/12 md:w-1/2 lg:w-2/5 xl:w-1/3 bg-base-100"
	>
		<form wire:submit.prevent="save" class="space-y-4">
			...
			<div class="flex justify-center py-2">
				<div class="relative">
					{{-- 頭像顯示 --}}
					<div class="h-24 w-24 rounded-full border-2 border-base-300 overflow-hidden bg-base-100 shadow-inner">
						@if ($photo)
						...
						@else
							<div class="h-full w-full flex flex-col items-center justify-center text-base-content/40">
			...
			<x-toggle label="帳號啟用狀態" wire:model="is_active" class="pt-2 text-base-content" />
