					...
					<div class="w-full md:w-1/2">
						<div class="flex items-center gap-3 min-w-0">
							...
							<div class="min-w-0 flex-1">
								<h3 class="text-base md:text-lg font-black text-stone-800 dark:text-stone-100 tracking-wide font-sans flex items-center gap-2 flex-wrap">
								...
								<p class="text-xs text-stone-500 dark:text-stone-400 mt-0.5 flex items-center gap-2 flex-wrap">
					...
					<div class="w-full md:w-1/2 flex md:justify-end">
						@if($accountId)
							...
							<div class="grid grid-cols-[60px_1fr] gap-x-4 text-right items-center mr-4 ml-auto">
								<span class="text-xs text-stone-400 dark:text-stone-500 text-left">期初</span>
								<span class="font-mono font-bold text-stone-600 dark:text-stone-300 text-sm">
									{{ $currencySymbol }}{{ number_format((float)($statsSummary['opening_balance'] ?? 0), 2) }}
								</span>
								
								<span class="text-xs text-emerald-500 dark:text-emerald-400 text-left">收入</span>
								...
								<span class="text-xs text-rose-500 dark:text-rose-400 text-left">支出</span>
								...
								<span class="text-xs font-bold text-stone-500 dark:text-stone-400 text-left border-t border-stone-200 dark:border-stone-700 pt-0.5">期末</span>
								@php
								...
								@endphp
								<span class="font-mono font-black text-sm {{ $closingClass }} border-t border-stone-200 dark:border-stone-700 pt-0.5">
						...
						@else
							...
							<div class="grid grid-cols-[60px_1fr] gap-x-4 text-right items-center mr-4 ml-auto">
								...
								<span class="text-xs font-bold text-stone-500 dark:text-stone-400 text-left border-t border-stone-200 dark:border-stone-700 pt-0.5">差額</span>
								@php
								...
								@endphp
								<span class="font-mono font-black text-sm {{ $netClass }} border-t border-stone-200 dark:border-stone-700 pt-0.5">
			...
			<div class="p-4 md:p-6 bg-base-100/40 dark:bg-stone-900/40 flex-1 overflow-y-auto min-h-0 space-y-4">
				...
				<div class="space-y-4 md:space-y-5 max-w-full">
					...
					<div class="w-full max-w-full flex-shrink-0">
						<div class="flex items-center justify-between bg-stone-100/80 dark:bg-stone-800/80 p-1.5 rounded-2xl border border-stone-200/60 dark:border-stone-700/60 w-full shadow-inner">
							<x-button 
								icon="o-chevron-left" 
								class="btn-xs btn-ghost text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-100 hover:bg-white dark:hover:bg-stone-700 px-2.5 h-7 min-h-0 rounded-xl transition-all shadow-sm" 
								wire:click="previousRange" 
							/>
							
							<div class="flex items-center gap-1.5 font-mono font-black text-xs md:text-sm text-stone-800 dark:text-stone-200 tracking-wider select-none">
					...
					<div class="space-y-1.5">
						@forelse($transactionsData['list'] ?? [] as $tx)
							...
							<div class="relative flex items-center py-2 px-3 bg-stone-50/50 hover:bg-stone-100 dark:bg-stone-800/40 dark:hover:bg-stone-800 rounded-lg transition-all duration-150 cursor-pointer group border border-stone-200 dark:border-stone-700/60"
								 wire:click="$dispatch('open-transaction-modal', { transactionId: {{ $tx['id'] }} })">
								
								<div class="rounded-xl p-1.5 md:p-2 flex-shrink-0 {{ $isTransfer ? 'bg-sky-100 dark:bg-sky-950/60' : ($isIncome ? 'bg-emerald-100 dark:bg-emerald-950/60' : ($isExpense ? 'bg-rose-100 dark:bg-rose-950/60' : 'bg-stone-200 dark:bg-stone-700')) }}">
									<x-dynamic-component 
								...
								<div class="flex-1 min-w-0 ml-2">
									<div class="flex items-center justify-between gap-2">
										<div class="flex items-center gap-1 min-w-0 truncate">
											<span class="text-sm md:text-sm font-medium text-stone-800 dark:text-stone-200 flex-shrink-0">
												{{ $subCategoryName }}
											</span>
											
											@if($accountName)
												<span class="text-xs text-stone-500 dark:text-stone-400 font-normal truncate">
													（{{ $accountName }}）
												</span>
											@endif
										</div>
									...
									<div class="flex items-center justify-between gap-2 mt-0.5">
										<span class="text-xs text-stone-500 dark:text-stone-400 truncate {{ $summary ? '' : 'italic' }}">
											{{ $summary ?: '無備註' }}
										</span>
										<span class="text-xs font-mono text-stone-400 dark:text-stone-500 flex-shrink-0">
						...
						@empty
							<div class="text-center text-stone-400 dark:text-stone-500 py-10 bg-stone-50/20 dark:bg-stone-900/20 rounded-xl border border-dashed border-stone-200 dark:border-stone-800">
								<x-heroicon-o-document-text class="w-10 h-10 mx-auto mb-2 opacity-25 text-stone-400 dark:text-stone-500" />
